...
extern "C" {
  ...
  static PyObject* call_global_staffline_skew_angle(PyObject* self, PyObject* args);
...
static PyMethodDef _staff_removal_fujinaga_methods[] = {
  ...
  { CHAR_PTR_CAST "global_staffline_skew_angle",
    call_global_staffline_skew_angle, METH_VARARGS,
    CHAR_PTR_CAST "**global_staffline_skew_angle** (float *staffline_height* = 0.00, float *staffspace_height* = 0.00, int *skew_strip_width* = 0, float *max_skew* = 5.00)\n\nPerforms a smooth deskewing on the entire image by\ncross-correlating thin vertical strips of the image.  Since this\nfunction is global and does not deskew on staff at a time, the results\nare not great, and in particular long slurs can introduce errors.\nThis same deskewing also happens early on in the\nfind_and_remove_staves_fujinaga process.\n\nReturns deskewed copy of the image.\n\n*staffline_height* = 0\n   The height (in pixels) of the stafflines. If *staffline_height* <=\n   0, the staffline height is autodetected both globally and for each\n   staff.\n\n*staffspace_height* = 0\n   The height (in pixels) of the spaces between the stafflines.  If\n   *staffspace_height* <= 0, the staffspace height is autodetected\n   both globally and for each staff.\n\n*skew_strip_width* = 0\n   The width (in pixels) of vertical strips used to deskew the image.\n   Smaller values will deskew the image with higher resolution, but\n   this may make the deskewing overly sensitive.  Larger values may\n   miss fine detail.  If *skew_strip_width* <= 0, the width will be\n   autodetermined to be the global *staffspace_h* * 2.\n\n*max_skew* = 8.0\n   The maximum amount of skew that will be detected within each\n   vertical strip.  Expressed in degrees.  This value should be fairly\n   small, because deskewing only approximates rotation at very small\n   degrees.\n"},
...
static PyObject* call_global_staffline_deskew(PyObject* self, PyObject* args) {
  ...
  if (return_arg == NULL) {
    ...
    return_pyarg = create_ImageObject(return_arg);
    return return_pyarg;
  }
}
...
static PyObject* call_global_staffline_skew_angle(PyObject* self, PyObject* args) {
  ...
  if (PyArg_ParseTuple(args, CHAR_PTR_CAST "Oddid:global_staffline_skew_angle"
          ,
          &self_pyarg,
          &staffline_height_arg,
          &staffspace_height_arg,
  ...
  try {
    switch (get_image_combination(self_pyarg)) {
      case ONEBITIMAGEVIEW:
        return_arg = global_staffline_skew_angle(*((OneBitImageView*) self_arg), staffline_height_arg, staffspace_height_arg, skew_strip_width_arg, max_skew_arg);
        break;
      case CC:
        return_arg = global_staffline_skew_angle(*((Cc*) self_arg), staffline_height_arg, staffspace_height_arg, skew_strip_width_arg, max_skew_arg);
        break;
      case ONEBITRLEIMAGEVIEW:
        return_arg = global_staffline_skew_angle(*((OneBitRleImageView*) self_arg), staffline_height_arg, staffspace_height_arg, skew_strip_width_arg, max_skew_arg);
        break;
      case RLECC:
        return_arg = global_staffline_skew_angle(*((RleCc*) self_arg), staffline_height_arg, staffspace_height_arg, skew_strip_width_arg, max_skew_arg);
        break;
      case MLCC:
        return_arg = global_staffline_skew_angle(*((MlCc*) self_arg), staffline_height_arg, staffspace_height_arg, skew_strip_width_arg, max_skew_arg);
        break;
      default:
        PyErr_Format(PyExc_TypeError, "The 'self' argument of 'global_staffline_skew_angle' can not have pixel type '%s'. Acceptable values are ONEBIT, ONEBIT, ONEBIT, ONEBIT, and ONEBIT.", get_pixel_type_name(self_pyarg));
        return 0;
    }
  } catch (std::exception& e) {
    PyErr_SetString(PyExc_RuntimeError, e.what());
